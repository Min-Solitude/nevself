...
import { createInfoProfile } from "@/store/reducer/auth/auth.reducer";
...
export default function CreateProfile(props: Props) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      story: (e.currentTarget.story as HTMLInputElement).value,
      skills: isSkills,
      info: {
        location: (e.currentTarget.location as HTMLInputElement).value,
  ...
  return (
    ...
    >
      <div className="w-full flex max-w-[30rem] m-auto justify-end">
        <Button kind="primary" className="px-4" onClick={props.close}>
          <span className="font-medium">Đóng</span>
        </Button>
      </div>
      {props.uid_creator === props.profile?.uid ? (
        <div className="w-full mt-4 max-w-[30rem] m-auto">
          <h1 className="font-bold text-xl text-gray-800">Tạo thông tin</h1>
          ...
          >
            ...
            {loading ? (
              ...
              <Button type="submit" kind="primary-dark" className="mt-4">
                <span className="font-medium">Tạo thông tin</span>
              </Button>
            )}
          </form>
        </div>
